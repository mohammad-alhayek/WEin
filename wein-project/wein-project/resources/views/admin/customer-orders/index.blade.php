...
<div class="customer-header-card mb-4">
    <div class="d-flex justify-content-between align-items-lg-center flex-column flex-lg-row gap-4">
        <div>
            <h1 class="customer-page-title">{{ __('messages.customer_orders') }}</h1>
            <p class="customer-page-subtitle mb-0">Manage and monitor customer orders</p>
        </div>
        {{-- Server-side search --}}
        <form method="GET" action="{{ route('admin.customer-orders.index') }}" class="co-search-form">
            ...
            <button type="submit" class="btn btn-secondary btn-sm" style="white-space:nowrap">{{ __('messages.search') }}</button>
            @if(request('search'))
                <a href="{{ route('admin.customer-orders.index') }}" class="btn btn-secondary btn-sm">✕</a>
            @endif
        </form>
    </div>
</div>
        ...
        <div class="customer-card">
            <div class="customer-card-body">
                ...
                <div class="d-flex justify-content-between align-items-start mb-4">
                    <div>
                        <div class="customer-name">{{ $co->customer_name }}</div>
                        <div class="customer-id">#{{ $co->id }}</div>
                    </div>
                    @if($co->is_updated)
                        <span class="status-badge updated">{{ __('messages.order_modified') }}</span>
                    @endif
                </div>
                ...
                <div class="customer-info-grid">
                    <div class="info-box">
                        <span class="info-label">{{ __('messages.phone_number') }}</span>
                        <span class="info-value">{{ $co->phone_number }}</span>
                    </div>
                    <div class="info-box">
                        <span class="info-label">{{ __('messages.order') }}</span>
                        <span class="info-value">{{ $co->order->title ?? '—' }}</span>
                    </div>
                    <div class="info-box">
                        <span class="info-label">{{ __('messages.price') }}</span>
                        <span class="info-value {{ $co->price > 0 ? 'price-set' : 'price-pending' }}">
                            @if($co->price > 0)
                                {{ number_format($co->price, 2) }}
                            @else
                    ...
                    <div class="info-box">
                        <span class="info-label">{{ __('messages.total_price') }}</span>
                        <span class="info-value">{{ number_format($co->total_price, 2) }}</span>
                    </div>
                </div>
                ...
                <div class="meta-section">
                    <div>
                        ...
                        <div>{{ $co->created_at->format('d M Y') }}</div>
                    </div>
                    <div>
                ...
                <div class="actions-wrapper">
                    <a href="{{ route('admin.customer-orders.show', $co->id) }}" class="btn-view">
                        {{ __('messages.view') }}
                    </a>
                    <form id="del-co-{{ $co->id }}" method="POST" action="{{ route('admin.customer-orders.destroy', $co->id) }}">
                        @csrf
                        @method('DELETE')
