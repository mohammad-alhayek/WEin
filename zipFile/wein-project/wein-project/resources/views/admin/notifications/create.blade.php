...
@section('title', __('messages.create') . ' ' . __('messages.notification'))

@section('content')
<div class="page-header">
    <h1>{{ __('messages.create') }} {{ __('messages.notification') }}</h1>
        ...
        <form method="POST" action="{{ route('admin.notifications.store') }}">
            ...
            <div class="form-group">
                ...
                <select name="order_id" class="form-control" required>
                    ...
                    @foreach($orders as $o)
                        <option value="{{ $o->id }}" {{ old('order_id') == $o->id ? 'selected' : '' }}>{{ $o->title }}</option>
                    @endforeach
                </select>
